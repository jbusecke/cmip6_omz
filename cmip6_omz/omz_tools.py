...
from .units import convert_mol_m3_mymol_kg, convert_o2_ml_l
...
def mask_basin(ds, region="Pacific", drop=True):
    ...
    basins = (
        regionmask.defined_regions.natural_earth_v4_1_0.ocean_basins_50
    )  # TODO: I should update this to v5 at some point
...
def volume_consistency_checks(ds_z, ds_sigma):
    ...
    omz_vol_post = omz_full_volume(ds_sigma).sum(["sigma_0", "x", "y"])
...
def omz_thickness_efficient(
    ...
    o2_bins_converted = xr.DataArray(
        o2_bins_converted_raw, dims=["o2_bin"], coords={"o2_bin": o2_bins}
    )  # can I generalize this with pint?
...
def sigma_bins():
    ...
    """
    return np.hstack(
        [[0], np.arange(22.5, 26.5, 0.25), np.arange(26.5, 27.9, 0.05), [100]]
    )
...
def align_missing(ds_in):
    ...
    if "time" in ds_in.dims:
        ds_mask = xr.concat(
            [ds_in.isel(time=slice(0, 2)), ds_in.isel(time=slice(-2, None))], "time"
        )
    else:
        ds_mask = ds_in
    ...
    combo_nanmask = np.logical_or(
        ...
        np.isnan(ds_mask.thetao)
        .all("time" if "time" in ds_in.dims else [])
        .load(),  # the `rho` function returns nan when one of t/s is nan, so we only need to chcek one of them
    )
    try:
        plt.figure()
...
def preprocessing_wrapper(ds_in):
    ...
    drop_vars = [
        va
        for va in ds_in.data_vars
        if va not in ["o2", "agessc", "so", "thetao", "sigma_0", "omz_thickness"]
    ]
...
def vol_consistency_check_wrapper(ds, ds_sigma):
    perc_difference, omz_perc_difference = volume_consistency_checks(ds, ds_sigma)
    print(
    ...
    if (abs(perc_difference) > 0.1).any() or (
        abs(omz_perc_difference)
        > 0.25  # Had to increase for ESM4 before most would pass 0.01
    ).any():

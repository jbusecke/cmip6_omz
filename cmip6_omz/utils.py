...
def _pick_latest_version_aggressive(df):
    ...
    grpby = list(
        set(df.columns.tolist()) - {"path", "version", "dcpp_init_year", "time_range"}
    )  # I cannot do this generally, because then we throw out actual data
    ...
    def _pick_latest_v(group):
        ...
        if group.version.nunique() > 1:
            idx = (
                group.sort_values(by=["version"], ascending=False)
                .index[1:]
                .values.tolist()
            )
        return idx

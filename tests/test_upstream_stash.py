...
from cmip6_omz.upstream_stash import construct_static_dz, xgcm_transform_wrapper
...
def dummy_ds():
    ...
    da_sigma0 = xr.DataArray(
        (24.0 + 5.0 * np.random.random((nt, nz, ny, nx))),
        dims=["time", "lev", "y", "x"],
        coords={"x": x, "y": y, "lev": lev, "time": t},
...
def test_xgcm_transform_wrapper(dummy_ds):
    ...
    ds_sigma = xgcm_transform_wrapper(ds, target=sigma_bins, target_data=ds.sigma_0)
...
def test_xgcm_transform_wrapper2(dummy_ds):
    ...
    ds_sigma = xgcm_transform_wrapper(
        ds, extensive_vars=["omz_thickness"], target=sigma_bins, target_data=ds.sigma_0
    )
    ...
    vol_sigma = (
        (ds_sigma.omz_thickness * ds_sigma.areacello).sum(["x", "y", "sigma_0"]).load()
    )

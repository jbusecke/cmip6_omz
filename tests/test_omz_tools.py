...
def dummy_ds():
    ...
    np.random.seed(0)
    ds = xr.DataArray(
        (300.*1e6 / 1025.)*np.random.random((nt, n_sig, ny, nx)),
...
def test_omz_thickness(dummy_ds, o2_bins):
    ...
    dz = xr.ones_like(o2)
    thickness_test = [dz.where(o2 <= o2_bins, 0).assign_coords(o2_bin=o2_bins).astype(o2.dtype)]
...
def test_mask_basin(basins):
    ...
    np.random.seed(0)
    ds = xr.DataArray(
        (300.*1e6 / 1025.)*np.random.random((n_sig, ny, nx)),
